...
from compare_with_reference import parse_reference_file, load_our_results
...
SPECIES_CONFIG = [
    {
        "name": "flies",
        "reference_file": "flyCBASsigSeq.txt",
        "max_seq_len": 10,
        "title": "Fly CBAS: CA vs w1118",
    },
    {
        "name": "humans",
        "reference_file": "humanCBASsigSeq.txt",
        "max_seq_len": 4,
        "title": "Human CBAS: Correlative with CBIT",
    },
    {
        "name": "rats",
        "reference_file": "ratSigSeq.txt",
        "max_seq_len": 6,
...
def plot_with_comparison(species_cfg, alpha=0.5):
    ...
    reference = parse_reference_file(
        NOTES_DIR / species_cfg["reference_file"],
    ...
    neither = set(all_seqs) - our_sig - reference_sig
    ...
    for seq in all_seqs:
        ...
        elif seq in only_reference:
            categories[i] = "reference_only"
        else:
    ...
    overlap_pct = len(both) / len(reference_sig) * 100 if reference_sig else 0
    style = {
        "both": {"color": "#2ca02c", "marker": ".", "s": 12,
                 "label": f"Both significant ({len(both)}, {overlap_pct:.1f}% of Igor)"},
        "reference_only": {"color": "#d94801", "marker": "x", "s": 24,
                       "label": f"Igor only ({len(only_reference)})"},
    ...
    for cat in ["neither", "both", "us_only", "reference_only"]:
        mask = ranked_cat == cat
        if not np.any(mask):
            continue
        ax_rank.scatter(
            ...
            zorder=3 if cat in ("reference_only", "us_only") else 2,
...
def main():
    for cfg in SPECIES_CONFIG:
        ...
        if not npz.exists():
            ...
            continue
        if cfg["reference_file"] and (NOTES_DIR / cfg["reference_file"]).exists():
