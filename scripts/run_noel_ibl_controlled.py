"""
CBAS on Noel et al. 2025 IBL mice — controlled analysis.

Uses ALL trials (all contrasts, all blocks) from the all_contrasts/ txt files,
truncated to a common per-animal criterion to eliminate exposure confound.

Encoding: choice × correctness → 4 symbols (L✓, L✗, R✗, R✓).

Usage:
    pixi run noel-ibl-ctrl               # L=6, criterion=4000
    pixi run noel-ibl-ctrl-quick         # M=1000
"""
...
DATA_DIR = ROOT_DIR / "data" / "noel_ibl_mice" / "all_contrasts"
RESULTS_DIR = ROOT_DIR / "results" / "noel_ibl_mice_controlled"
...
def load_data(criterion=4000):
    """Load all-contrast txt files, truncated to criterion."""
    ...
    for txt_path in sorted(DATA_DIR.glob("*.txt")):
        ...
        left_bias = ((symbols == 0) | (symbols == 1)).mean()
        stats["animals"][aid] = {
            "genotype": genotype,
...
def run_analysis(seq_len=6, criterion=4000, quick=False):
    print("=" * 70)
    print("CBAS — Noel IBL: All Trials, Criterion-Truncated")
    print("=" * 70)
    ...
    for g in ["WT", "Cntnap2", "Fmr1", "Shank3B"]:
        ...
        if n_mice > 0:
            ...
            biases = [s["left_bias"] for s in stats["animals"].values() if s["genotype"] == g]
            print(f"  {g:8s}: {n_mice:2d} mice × {criterion} trials, "
                  f"acc={np.mean(accs)*100:.1f}% ± {np.std(accs)*100:.1f}%, "
                  f"left={np.mean(biases)*100:.1f}%")
    ...
    for grp0_name, grp1_name in comparisons:
        ...
        results_json = {
            "dataset": "noel_ibl_mice_controlled",
            "comparison": f"{grp0_name}_vs_{grp1_name}",
            "filter": "all trials (all contrasts, excl no-go), criterion-truncated",
...
def make_figures(all_results, params):
    ...
    for key, result in all_results.items():
        ...
        fig, ax = manhattan_plot(
            ...
            title=f"Noel IBL controlled: {key} ({n_sig} sig / {result['n_seq']:,} seq)"
        )
        fig.savefig(FIG_DIR / f"manhattan_{key.lower()}.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
